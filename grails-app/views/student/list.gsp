                ...
                <table class="table table-striped">
                    ...
                    <g:each in="${studentInstanceList}" status="i" var="studentInstance">
                        <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                            ...
                            <td>
                                <g:link class="btn btn-info btn-small" action="show" params="${[studentId: fieldValue(bean: studentInstance, field: 'studentId')]}">${message(code: 'default.button.show.label', default: 'View')}</g:link>
                                <sec:ifAnyGranted roles="MANAGE_COURSEINFO_ROLE">
                                    <g:link class="btn btn-danger btn-small handle-delete" action="delete" data-id="${fieldValue(bean: studentInstance, field: 'studentId')}">${message(code: 'default.button.delete.label', default: 'Delete')}</g:link>
                                </sec:ifAnyGranted>
                           	</td>
                        </tr>
                    </g:each>
